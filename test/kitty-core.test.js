...
const LAT_STRING = web3.utils.toVon('1', "lat");
const FINNEY_STRING = web3.utils.toVon('1', "millilat");
const LAT_BN = new BigNumber(LAT_STRING);
const FINNEY_BN = new BigNumber(FINNEY_STRING);
const NULL_ADDRESS = "0x0000000000000000000000000000000000000000";

const util = require("./util.js");
// add test wrapper to make tests possible
const KittyCore = artifacts.require("./test/contracts/KittyCoreTest.sol");
...
contract("KittyCore", function(accounts) {
  ...
  describe("Roles: CEO + CFO", async function() {
    ...
    it("CFO can drain funds", async function() {
      await coreC.fundMe({ value: web3.utils.toVon('0.05', "lat") });
      const ctoBalance1 = web3.platon.getBalance(cfo);
      debug("cfo balance was", ctoBalance1);
      await coreC.withdrawBalance({ from: cfo });
      const ctoBalance2 = web3.platon.getBalance(cfo);
  ...
  describe("Auction wrapper", function() {
    ...
    it("should be able to bid on sale auction", async function() {
      const cooBal1 = await web3.platon.getBalance(coo);
      await saleAuction.bid(kittyId1, { from: user1, value: 200 });
      const cooBal2 = await web3.platon.getBalance(coo);
      const kitty1Owner = await coreC.ownerOf(kittyId1);
      eq(kitty1Owner, user1);
  ...
  describe("Gen0 Auction", function() {
    ...
    const kittyId1 = 1,
      kittyId2 = 2;
    const startingPrice = FINNEY_BN.times(10);
    ...
    it("next 3 gen0 auctions should be startingPrice", async function() {
      ...
      for (let id = 2; id < 5; id++) {
        ...
        assert(auction[1].eq(FINNEY_BN.times(10)));
        await saleAuction.bid(id, { from: user1, value: FINNEY_BN.times(10) });
        const avePrice = await saleAuction.averageGen0SalePrice();
      }
    });
  ...
  describe("auction withdrawals", function() {
    beforeEach(async function() {
      ...
      await coreC.setCFO(cfo, { from: ceo });
      // Get some LAT into both sale and siring auctions
    ...
    it("should be able to withdraw as coo", async function() {
      const saleBal1 = web3.platon.getBalance(saleAuction.address);
      const sireBal1 = web3.platon.getBalance(siringAuction.address);
      const coreBal1 = web3.platon.getBalance(coreC.address);
      await saleAuction.withdrawBalance({ from: coo });
      await siringAuction.withdrawBalance({ from: coo });
      const saleBal2 = web3.platon.getBalance(saleAuction.address);
      const sireBal2 = web3.platon.getBalance(siringAuction.address);
      const coreBal2 = web3.platon.getBalance(coreC.address);
      assert(
        coreBal1
          .add(saleBal1)
          .add(sireBal1)
