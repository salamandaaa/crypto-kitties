...
contract("Clock Auction", function(accounts) {
  ...
  const owner = 'lat1mph9ef447m8dd5cpa5792yzv3ldzexd9c5rcsz';
  const user1 = 'lat1kvprlyg44kuzgl3eqzu7fs0gsgg7903angx92g';
  const user2 = 'lat1q8s2eekw3kc6f5tnvqs8l9lt4xdp7kxe6rlx7x';
  ...
  describe.skip("Current price computation", function() {
    // Helpers
    let toVon, currentPrice;
    before(async function() {
      await deploy();
      toVon = num => web3._extend.utils.toVon(new BN(num), "lat");
    ...
    it("Big numbers", async function() {
      const [start, end, dur] = [toVon(100), toVon(200), 100000];
      let price = await currentPrice(start, end, dur, 0);
      assert(price.eq(start));
      price = await currentPrice(start, end, dur, 10000);
      assert(price.eq(toVon(110)));
      price = await currentPrice(start, end, dur, 60000);
      assert(price.eq(toVon(160)));
