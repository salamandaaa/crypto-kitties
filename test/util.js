...
const getBalance = async addr => {
  const res = await promisify(cb => web3.platon.getBalance(addr, cb));
  return new BigNumber(res);
};

const getGasPrice = () => {
  return promisify(web3.platon.getGasPrice);
};
...
const measureGas = async accounts => {
  let balanceSum = new BigNumber(0);
  // only checks the first 8 accounts
  for (let i = 0; i <= 7; i++) {
    balanceSum = balanceSum.plus(await getBalance(accounts[i]));
  }
